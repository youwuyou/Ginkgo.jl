...
function slab_breakoff()
    ...
    for timestep in 1:1:ntimesteps
        ...
        # visualization
        if DO_VIZ
            p1 = heatmap(xvy, yvy, RHOvy, aspect_ratio=1, xlims=(extrema(xvy)), ylims=(extrema(yvy)), c=:turbo, ticks = false, title="Density " * L"\rho, kg/m^3", interpolate = true)
            p2 = heatmap(x, y, log10.(ETAb), aspect_ratio=1, xlims=(extrema(x)), ylims=(extrema(y)), c=:turbo, ticks = false,  title=L"\log_{10}(ETAb), Pa\cdot s", interpolate = true)
            p3 = heatmap(xpr, ypr,  Tdt, aspect_ratio=1, xlims=(extrema(xpr)), ylims=(extrema(ypr)), c=:turbo, ticks = false, title="Temperature, K", interpolate = true)
            p4 = heatmap(xpr, ypr, Pr, aspect_ratio=1, xlims=(extrema(xpr)), ylims=(extrema(ypr)), c=:turbo, ticks = false, title="Pressure, Pa", interpolate = true)
            p5 = heatmap(xvx, yvx, Vx, aspect_ratio=1, xlims=(extrema(xvx)), ylims=(extrema(yvx)), c=:turbo, ticks = false, title="Vx, m/s", interpolate = true)
            p6 = heatmap(xvy, yvy, Vy, aspect_ratio=1, xlims=(extrema(xvy)), ylims=(extrema(yvy)), c=:turbo, ticks = false, title="Vy, m/s", interpolate = true)
            p7 = heatmap(xpr, ypr,  RHOcpp, aspect_ratio=1, xlims=(extrema(xpr)), ylims=(extrema(ypr)), c=:turbo, ticks = false, title="RHOcpp, " * L"J/K/m^3")
            p8 = heatmap(xvx, yvx,  log10.(Kvx), aspect_ratio=1, xlims=(extrema(xvx)), ylims=(extrema(yvx)), c=:turbo, ticks = false, title=L"\log_{10}(Kvx), W/m/K", interpolate = true)
            p9 = heatmap(xpr, ypr,  ALPHAp, aspect_ratio=1, xlims=(extrema(xpr)), ylims=(extrema(ypr)), c=:turbo, ticks = false, title="ALPHA, 1/K", interpolate = true)
            p10 = heatmap(xpr, ypr,  Hrp, aspect_ratio=1, xlims=(extrema(xpr)), ylims=(extrema(ypr)), c=:turbo, ticks = false, title="Hr, " * L"W/m^3", interpolate = true)
            p11 = heatmap(xpr, ypr,  Hsp, aspect_ratio=1, xlims=(extrema(xpr)), ylims=(extrema(ypr)), c=:turbo, ticks = false, title="Hs, " * L"W/m^3", interpolate = true)
            p12 = heatmap(xpr, ypr,  Hap, aspect_ratio=1, xlims=(extrema(xpr)), ylims=(extrema(ypr)), c=:turbo, ticks = false, title="Ha, " * L"W/m^3", interpolate = true)
